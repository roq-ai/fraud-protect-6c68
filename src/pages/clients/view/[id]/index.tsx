...
import { deletePhoneNumberById } from 'apiSdk/phone-numbers';
import { deleteSpoofCallById } from 'apiSdk/spoof-calls';
...
function ClientViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ClientInterface>(
    ...
    () =>
      getClientById(id, {
        relations: ['user', 'phone_number', 'spoof_call'],
      }),
  );

  const phone_numberHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
  ...
  const spoof_callHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('phone_number', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/phone-numbers/create?client_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.phone_number?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/phone-numbers/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.number}</Td>
                            <Td>
                              {hasAccess('phone_number', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/phone-numbers/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('phone_number', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    phone_numberHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
              ...
              {hasAccess('spoof_call', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/spoof-calls/create?client_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.spoof_call?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/spoof-calls/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.status}</Td>
                            <Td>
                              {hasAccess('spoof_call', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/spoof-calls/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('spoof_call', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    spoof_callHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
